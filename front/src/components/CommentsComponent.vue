...
export default {
  ...
  methods: {
    fetchComments(recipeId) {
      fetch(`http://localhost:3000/api/comments/${recipeId}`)
        ...
        .then((data) => {
          ...
          const userPromises = data.map((comment) =>
            this.fetchUser(comment.authorid)
          );
    ...
    async checkIfLiked(commentId) {
      try {
        const response = await fetch(
          `http://localhost:3000/api/likes/comment/check/${commentId}`
        );
    ...
    async deleteLike(commentId) {
      try {
        await fetch(
          `http://localhost:3000/api/likes/comment/delete/${commentId}`,
          {
            method: "DELETE",
            headers: {
    ...
    async addLike(commentId) {
      ...
      try {
        const response = await fetch(
          "http://localhost:3000/api/likes/comment/add",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
