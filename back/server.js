...
app.post("/auth/signup", async (req, res) => {
  try {
    ...
    const authUser = await pool.query(
      "INSERT INTO users(email, password, name, urllink, description) values ($1, $2, $3, $4, $5) RETURNING*",
      [email, bcryptPassword, name, urllink, description]
    );
...
app.put("/api/users/:id", async (req, res) => {
  try {
    console.log("Update user information request has arrived");
    const { id } = req.params;
...
app.get("/api/users/:id", async (req, res) => {
  try {
    console.log("Find user information request has arrived");
    const { id } = req.params;
...
app.get("/api/myrecipes", async (req, res) => {
  try {
    console.log("Get active user recipes request has arrived");
    const recipes = await pool.query(
      "SELECT * FROM recipestable WHERE authorid = $1",
      [activeUserId]
    );
    res.json(recipes.rows);
  } catch (err) {
...
app.get("/api/recipes/:id", async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const post = await pool.query("SELECT * FROM recipestable WHERE id = $1", [
      id,
    ]);
...
app.put("/api/recipes/:id", async (req, res) => {
  try {
    console.log("Update recipe request has arrived");
    const { id } = req.params;
...
app.post("/api/likes/add", async (req, res) => {
  try {
    console.log("Like post request has arrived");
    ...
    await pool.query(
      "UPDATE recipestable SET likes = likes + 1 WHERE id = $1",
      [recipe_id]
    );
...
app.delete("/api/likes/delete/:recipe_id", async (req, res) => {
  try {
    const { recipe_id } = req.params;
    console.log(`Delete like request has arrived`);
    await pool.query(
...
app.get("/api/comments/:recipe_id", async (req, res) => {
  try {
    ...
    const comments = await pool.query(
      "SELECT * FROM recipecommenttable WHERE recipe_id = $1",
      [recipe_id]
    );
...
app.post("/api/comments", async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE recipestable SET comments = comments + 1 WHERE id = $1",
      [comment.recipe_id]
    );
...
app.delete("/api/comments/:comment_id", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM recipecommenttable WHERE id = $1", [
      comment_id,
    ]);
...
app.get("/api/likes/comment/check/:comment_id", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT EXISTS (SELECT * FROM commentlikes WHERE user_id = $1 AND comment_id = $2)",
      [activeUserId, comment_id]
    );
    const likeExists = result.rows[0].exists;
...
app.post("/api/likes/comment/add", async (req, res) => {
  try {
    console.log("Like comment request arrived");
    const { comment_id } = req.body;
    await pool.query(
      "INSERT INTO commentlikes (user_id, comment_id) VALUES ($1, $2)",
      [activeUserId, comment_id]
    );
    await pool.query(
      "UPDATE recipecommenttable SET likes = likes + 1 WHERE id = $1",
      [comment_id]
    );
...
app.delete("/api/likes/comment/delete/:comment_id", async (req, res) => {
  try {
    const { comment_id } = req.params;
    console.log(
      `Delete like from comment id ${comment_id} request has arrived`
    );
    await pool.query(
      "DELETE FROM commentlikes WHERE user_id = $1 AND comment_id = $2",
      [activeUserId, comment_id]
    );
    await pool.query(
      "UPDATE recipecommenttable SET likes = likes - 1 WHERE id = $1",
